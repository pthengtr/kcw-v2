...
import type { BiExpenseItemMonthRow } from "@/lib/bi/expense-types";
import { formatBaht } from "@/lib/bi/sales-format";
import { formatThaiPeriodLabel } from "@/lib/bi/sales-periods";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Input } from "@/components/ui/input";

type ExpenseMonthCompareTableProps = {
  monthColumns: string[];
  rows: BiExpenseItemMonthRow[];
...
export default function ExpenseMonthCompareTable({
  monthColumns,
  rows,
  trendTotals,
}: ExpenseMonthCompareTableProps) {
  ...
  const columnTotals = useMemo(() => {
    ...
    for (const period of monthColumns) {
      if (trendTotals && period in trendTotals) {
        totals[period] = trendTotals[period] ?? 0;
        continue;
      }
    ...
    return totals;
  }, [filtered, monthColumns, trendTotals]);
      ...
      <CardHeader className="gap-3 pb-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            แถว = ประเภทค่าใช้จ่าย · คอลัมน์ = เดือน (YTD)
          </p>
        </div>
        <Input
          value={query}
              ...
              {filtered.length === 0 ? (
                ...
                <>
                  {filtered.map((row) => (
                    ...
                    >
                      <td className="sticky left-0 z-10 max-w-[12rem] bg-white py-2 pr-3">
