...
import { enqueueInventorySync } from "@/lib/po/worker-jobs";
...
export async function POST() {
  ...
  try {
    ...
    const result = await enqueueInventorySync({
      supabase,
    ...
    console.error("inventory sync enqueue", error);
    return NextResponse.json(
      { error: "Unable to enqueue inventory sync" },
      { status: 500 }
    );
  }
}
