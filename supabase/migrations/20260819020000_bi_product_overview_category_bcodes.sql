...
-- Optional p_category (KACC9 / BCODE prefix) and p_bcodes (custom SKU set).
...
DROP FUNCTION IF EXISTS public.fn_bi_product_overview(date, date, text, integer);
...
BEGIN
  ...
  SELECT array_agg(DISTINCT btrim(x))
    INTO v_bcodes
  FROM unnest(COALESCE(p_bcodes, ARRAY[]::text[])) AS x
  WHERE btrim(COALESCE(x, '')) <> '';

  IF v_bcodes IS NOT NULL AND cardinality(v_bcodes) > 12 THEN
    v_bcodes := v_bcodes[1:12];
  END IF;
  ...
  line_base AS (
    ...
    WHERE l."IS_VALID" = 'True'
      ...
      AND (v_category IS NULL OR lpad(left(COALESCE(l."BCODE", ''), 2), 2, '0') = v_category)
      AND (v_bcodes IS NULL OR nullif(btrim(l."BCODE"), '') = ANY (v_bcodes))
  ),
  prev_lines AS (
    SELECT
      CASE
...
COMMENT ON FUNCTION public.fn_bi_product_overview(date, date, text, integer, text, text[]) IS
  'Product BI: line net revenue ranking by BCODE with category/CODE1 splits; optional p_category / p_bcodes.';

REVOKE ALL ON FUNCTION public.fn_bi_product_overview(date, date, text, integer, text, text[]) FROM PUBLIC, anon, authenticated;
