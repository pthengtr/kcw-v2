-- SYP ICLOW: optional p_prepare filter (all / not_prepared / partially_prepared / prepared)
-- on BCODE-level prepare_status from fn_po_syp_tf_prepare_by_bcode.
...
drop function if exists public.fn_po_pending_receive(text, text, text, text, text, text, integer, integer, integer, text);
...
declare
  ...
  v_prepare text := lower(btrim(coalesce(p_prepare, 'all')));
  v_result jsonb;
begin
  ...
  if v_prepare not in ('all', 'prepared', 'partially_prepared', 'not_prepared') then
    raise exception 'invalid prepare filter: %', p_prepare;
  end if;
    ...
    if v_site = 'HQ' then
      ...
      filtered as (
        ...
        where (v_vendor is null or coalesce(b.vendor, '') = v_vendor)
          and (
            v_prepare = 'all'
            or coalesce(b.prepare_status, 'not_prepared') = v_prepare
          )
          and (
            v_q is null
            or coalesce(b.docno, '') ilike '%' || v_q || '%'
  ...
  -- ค้างรับ / รับบางส่วน / รับแล้ว: DOCNO+BCODE grain
    ...
    filtered as materialized (
      ...
      where (
          v_prepare = 'all'
          or coalesce(c.prepare_status, 'not_prepared') = v_prepare
        )
        and (
          v_q is null
          or coalesce(c.docno, '') ilike '%' || v_q || '%'
...
revoke all on function public.fn_po_pending_receive(text, text, text, text, text, text, integer, integer, integer, text) from public;
grant execute on function public.fn_po_pending_receive(text, text, text, text, text, text, integer, integer, integer, text) to service_role;
