...
-- p_as_of (Bangkok calendar date) shifts "today" / 7-day / 14-day windows.

drop function if exists public.fn_stock_work_kpi(text);
...
declare
  v_branch text;
  v_bangkok_today date := public._stock_audit_bangkok_today();
  v_today date;
  v_week_start date;
begin
  ...
  v_today := coalesce(p_as_of, v_bangkok_today);
  if v_today > v_bangkok_today then
    v_today := v_bangkok_today;
  end if;
...
revoke all on function public.fn_stock_work_kpi(text, date) from public, anon, authenticated;
grant execute on function public.fn_stock_work_kpi(text, date) to service_role;
