  ...
  report_line_defs AS (
    ...
    FROM (
      VALUES
        ...
        ('net_cash', 'เงินสดสุทธิ', 'net', 7),
        ('ending_cash', 'เงินสดคงเหลือ', 'balance', 8)
    ) AS t(key, label, kind, sort_order)
  ),
  net_cash_month AS (
    SELECT
      mb.period,
      COALESCE(SUM(rgm.amount) FILTER (
        WHERE rgm.key IN ('sales_in', 'ar_in')
      ), 0)
      - COALESCE(SUM(rgm.amount) FILTER (
        WHERE rgm.key IN ('supplier_out', 'payroll_out', 'opex_out')
      ), 0) AS amount
    FROM month_bounds mb
    LEFT JOIN report_group_month rgm ON rgm.period = mb.period
    GROUP BY mb.period
  ),
  report_by_month AS (
    SELECT
      ...
      CASE
        ...
        WHEN d.key = 'net_cash' THEN COALESCE(
          (SELECT SUM(amount) FROM net_cash_month),
          0
        )
        ELSE COALESCE(
  ...
  SELECT jsonb_build_object(
    ...
    'report', (
      SELECT jsonb_build_object(
        ...
        'net_cash',
          ra.sales_in + ra.ar_in
          - ra.supplier_out - ra.payroll_out - ra.opex_out,
        ...
        'lines', jsonb_build_array(
          ...
          jsonb_build_object(
            'key', 'net_cash',
            'label', 'เงินสดสุทธิ',
            'amount', ra.sales_in + ra.ar_in - ra.supplier_out - ra.payroll_out - ra.opex_out,
            'kind', 'net',
            'line_count', NULL
          ),
