...
-- Implied BILLNO: same after stripping spaces (e.g. A219623 ↔ "A 219623") — match_method='pattern'.
-- Fallback (HQ only): when BILLNO miss — same AP + PIMAS.PO matches ICLOW.DOCNO
--   (PO6907-579 or 6907-579 via fn_po_docno_key) + exact BCODE/qty fingerprint.
--   Marked match_method='pattern' (implied / not 1:1). Do NOT use PIMAS.PO for membership alone.
...
-- Normalize HQ/SYP DOCNO / PIMAS.PO for compare: PO6907-579 and 6907-579 → 6907-579.
create or replace function public.fn_po_docno_key(p_docno text)
returns text
language sql
immutable
as $$
  select nullif(
    regexp_replace(upper(btrim(coalesce(p_docno, ''))), '^[0-9]*PO', ''),
    ''
  );
$$;

revoke all on function public.fn_po_docno_key(text) from public, anon, authenticated;
grant execute on function public.fn_po_docno_key(text) to service_role;
  ...
  if v_site = 'HQ' then
    ...
    unmatched_hdr as (
      ...
      where not exists (
        ...
        and not exists (
          select 1 from resolved_implied_billno e where e.rcvdno = r.rcvdno
        )
      group by r.rcvdno
      having max(i.vendor) is not null
    ),
    ...
    resolved as (
      ...
      union all
      select rcvdno, billno, match_method from resolved_implied_billno
      union all
