...
begin
  ...
  if v_site = 'HQ' and v_status = 'partially_received' then
    -- Line rows on mixed DOCNO: each ICLOW line + pending/received + PIMAS BILLNO
    with mixed as (
      select nullif(btrim(coalesce(i."DOCNO", '')), '') as docno
      from raw_kcw.raw_hq_iclow_stock_orders i
      where coalesce(i."CANCELED", 'N') <> 'Y'
        and i."ORDERED" = 'Y'
        and nullif(btrim(coalesce(i."DOCNO", '')), '') is not null
      group by 1
      having count(*) filter (where coalesce(i."RECEIVED", 'N') = 'N') > 0
         and count(*) filter (where i."RECEIVED" = 'Y') > 0
    ),
    enriched as (
      select
        i."ID" as id,
        ...
        case
          when coalesce(i."RECEIVED", 'N') = 'Y' then 'received'
          else 'pending'
        end as receive_state,
        left(i."RCVDDATE"::text, 10) as rcvddate,
        nullif(btrim(coalesce(i."RCVDNO", '')), '') as rcvdno,
        coalesce(
          nullif(btrim(coalesce(p."BILLNO", '')), ''),
          nullif(btrim(coalesce(i."RCVDNO", '')), '')
        ) as billno,
        left(p."BILLDATE"::text, 10) as billdate,
        'partially_received'::text as status,
        'line'::text as grain
      from mixed m
      join raw_kcw.raw_hq_iclow_stock_orders i
        on nullif(btrim(coalesce(i."DOCNO", '')), '') = m.docno
      left join raw_kcw.raw_hq_apmas_payable ap on ap."ACCTNO" = i."VENDOR"
      left join raw_kcw.raw_hq_pimas_purchase_bills p
        on p."BILLNO" = nullif(btrim(coalesce(i."RCVDNO", '')), '')
    ...
    filtered as (
      ...
      where (
        ...
        and (
          ...
          or coalesce(e.bcode, '') ilike '%' || v_q || '%'
          or coalesce(e.descr, '') ilike '%' || v_q || '%'
          or coalesce(e.rcvdno, '') ilike '%' || v_q || '%'
          or coalesce(e.billno, '') ilike '%' || v_q || '%'
        )
    )
    select jsonb_build_object(
      ...
      'rows', coalesce((
        select jsonb_agg(to_jsonb(r) order by
          r.docdate desc nulls last, r.docno, r.receive_state, r.bcode nulls last, r.id)
        from (
          select * from filtered
          order by docdate desc nulls last, docno, receive_state, bcode nulls last, id
    ...
    enriched as (
      select
        ...
        left(i."RCVDDATE"::text, 10) as rcvddate,
        nullif(btrim(coalesce(i."RCVDNO", '')), '') as rcvdno,
        -- SYP: no local PIMAS; expose ICLOW.RCVDNO as bill reference
        nullif(btrim(coalesce(i."RCVDNO", '')), '') as billno,
        left(i."RCVDDATE"::text, 10) as billdate,
        'partially_received'::text as status,
        'line'::text as grain
      from mixed m
      join raw_kcw.raw_syp_iclow_stock_orders i
        on nullif(btrim(coalesce(i."DOCNO", '')), '') = m.docno
      left join raw_kcw.raw_hq_apmas_payable ap on ap."ACCTNO" = i."VENDOR"
      where coalesce(i."CANCELED", 'N') <> 'Y'
        and i."ORDERED" = 'Y'
