-- SYP ICLOW tabs: expose prepare_status + prepare_tf_billnos from
-- fn_po_syp_tf_prepare_status() (same source as /po รายการ PO) so ค้างรับ
-- can show สถานะจัด + TF billnos and open the same drill-down dialog.
...
begin
  ...
  -- ค้างรับ / รับบางส่วน / รับแล้ว: DOCNO+BCODE grain
    ...
    classified as (
      ...
      from aggregated a
      left join raw_kcw.raw_hq_apmas_payable ap on ap."ACCTNO" = a.vendor
      left join public.fn_po_syp_tf_prepare_status() ps on ps.docno = a.docno
    ),
    filtered as materialized (
      select *
      from classified c
    ...
    classified as (
      ...
      left join raw_kcw.raw_hq_apmas_payable ap on ap."ACCTNO" = o.vendor
      left join public.fn_po_syp_tf_prepare_status() ps on ps.docno = o.docno
    ),
    filtered as materialized (
      select *
      from classified c
