/**
 * Multi-sheet statement parsing (no xlsx dependency — vitest-safe).
 * Workbook bytes are decoded in parser.ts, then each sheet grid is parsed here.
 */
import {
  columnsAreUsable,
  normCols,
...
export async function parseStatementSheets(
  sheets: SheetGrid[],
  opts: {
    filename: string;
    bankName: string;
  ...
  let carriedAccount = fallbackAccount || "";

  for (const { name: sheetName, grid } of sheets) {
    if (!grid.length) {
      sheetSummaries.push({
        sheet_name: sheetName,
        account_no: carriedAccount,
        row_count: 0,
        skipped: true,
      });
      continue;
    ...
    const sheetAccount = resolveSheetAccount({
      metadataAccount: metaAccount,
      sheetName,
      carriedAccount,
      fallbackAccount,
    });
    if (sheetAccount) carriedAccount = sheetAccount;

    const headerRow = findHeaderRow(grid);
    if (headerRow === null) {
      sheetSummaries.push({
        sheet_name: sheetName,
        account_no: sheetAccount,
        row_count: 0,
        skipped: true,
      });
      continue;
    ...
    const idx = (colName: string | null) => (colName ? uniqueCols.indexOf(colName) : -1);
    ...
    sheetSummaries.push({
      sheet_name: sheetName,
      account_no: sheetAccount,
      row_count: lines.length - before,
      skipped: lines.length === before,
    });
  }

  const accountNos = uniqueAccountNos(
    sheetSummaries.filter((s) => s.row_count > 0).map((s) => s.account_no),
  );
  meta.account_no = formatFileAccountNo(accountNos) ?? carriedAccount;
  meta.account_nos = accountNos;
  meta.sheet_summaries = sheetSummaries;
