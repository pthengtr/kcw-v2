...
- `NEXT_PUBLIC_LINE_LIFF_PRODUCT_SCANNER_ID` — leftover; LIFF product scan is **retired**. Do not use it for new LINE scan work.
...
- **`/liff/*`** is still a public path (legacy experiment). Product scan does **not** use it — LINE camera / album is handled in **kcw-api** (`docs/product-scan.md`). Do not add a second KCW login or new LIFF features.
