...
create index if not exists raw_syp_podet_docno_docdate_idx
  on raw_kcw.raw_syp_podet_purchase_order_lines ("DOCNO", "DOCDATE", "LINE");

drop function if exists public.fn_po_pending_receive(text, text, text, text, integer, integer, integer);
...
begin
  ...
  if v_site = 'HQ' then
    ...
    -- SYP: no PIMAS/PIDET ingest — treat open headers (BILLED <> Y) as fully pending
    with headers as (
      select
        h."DOCNO" as docno,
        h."DOCDATE" as docdate,
      ...
      where coalesce(h."CANCELED", '') <> 'Y'
        ...
        and (
          ...
          or exists (
            select 1
            from raw_kcw.raw_syp_podet_purchase_order_lines d0
            where d0."DOCNO" = h."DOCNO"
              and d0."DOCDATE" = h."DOCDATE"
              and coalesce(d0."CANCELED", '') <> 'Y'
              and (
    ...
    pending as (
      select
        ...
        d."UI" as ui,
        coalesce(d."QTY"::numeric, 0) as po_qty,
        0::numeric as recv_qty,
        coalesce(d."QTY"::numeric, 0) as remaining
      from headers h
      join raw_kcw.raw_syp_podet_purchase_order_lines d
        on d."DOCNO" = h.docno
       and d."DOCDATE" = h.docdate
       and coalesce(d."CANCELED", '') <> 'Y'
      where coalesce(d."QTY"::numeric, 0) > 0
        and (
          v_q is null
          or h.docno ilike '%' || v_q || '%'
          or coalesce(h.acctname, '') ilike '%' || v_q || '%'
...
revoke all on function public.fn_po_pending_receive(text, text, text, text, text, integer, integer, integer)
  from public, anon, authenticated;
grant execute on function public.fn_po_pending_receive(text, text, text, text, text, integer, integer, integer)
  to service_role;
