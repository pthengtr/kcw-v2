...
kcw-v2 **อัปโหลด Statement** calls Edge Function `import-bank-statement` in this repo (`auto_v2` fingerprints, RBAC `bank_statement_sync` or service-role for Drive bulk; no `ops.job_queue`). The daily HQ BAT refreshes BRDET/BPDET then uploads Drive Excel via the same Edge Function — see [worker-jobs](../worker-jobs.md#bank-statement-upload-from-bank-statement-sync-kcw-v2--not-a-pc-worker-job).
