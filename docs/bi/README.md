...
| [sql/fn_po_pending_receive.sql](./sql/fn_po_pending_receive.sql) | HQ pending-receive lines (PODET − linked PIDET by BCODE) |
...
- UI: `/po` — HQ/SYP purchase-order status + pending-receive trial tab (sync via `sync_pomas_podet`; SYP prepare overlay)
